...
(function() {
  try {
    ...
    var domain = script.src.match(/^https?:\/\/((?:\w+\.)*vk.com)/)[1];
    if (domain) {
      VK._domain.base = 'https://' + domain;
    }
  } catch (e) {}
})();
      ...
      obj[f] = function() {
        ...
        if (!VK._openApiAttached) {
          VK.callMethod('_getAppInfo', function(data) {
            var baseDomain = VK._domain && VK._domain.base && VK._domain.base.match(/^(\w+\.)*vk.com$/) ? VK._domain.base : 'https://vk.com';
            VK._apiId = data[0];
            VK._browserHash = data[1];
            VK.addScript(baseDomain + '/js/api/openapi.js?168');
          });
